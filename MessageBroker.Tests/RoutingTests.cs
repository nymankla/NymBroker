...
public sealed class RoutingTests
{
    ...
    private static (MessageBrokerImpl Broker, MemoryQueueEndPoint Dest, PriceConsumer Consumer) BuildBroker()
    {
    ...
    private static MessageBrokerImpl BuildScopedBroker()
    {
    ...
    public async Task EndPoint_RegisteredInDi_IsDisposedWithProvider()
    {
        ...
        services.AddSingleton<MessageBrokerImpl>(sp =>
        {
            var broker = new MessageBrokerImpl(
                sp.GetRequiredService<MessageSerializerJson>(),
                sp.GetRequiredService<IAggregator>(),
        ...
        {
            _ = sp.GetRequiredService<MessageBrokerImpl>();
    ...
    [Fact]
    public async Task AddMessageBroker_RegistersHostedService_ThatStartsAndStopsBroker()
    {
        var services = new ServiceCollection();
        services.AddLogging();
        ...
        services.AddMessageBroker().Build();

        await using var sp = services.BuildServiceProvider();

        var broker = (MessageBrokerImpl)sp.GetRequiredService<IMessageBroker>();
